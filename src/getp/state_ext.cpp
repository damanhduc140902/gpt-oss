...
void ext_alloc_device(int device_index, int batch_size, int expert_parallelism,
  ...
  HIP_CHECK(hipMalloc(&g_ext[device_index].local_ids,
                      sizeof(int) * (size_t)expert_parallelism * batch_size *
                          experts_per_token));
  HIP_CHECK(hipMalloc(&g_ext[device_index].local_wts,
                      sizeof(float) * (size_t)expert_parallelism * batch_size *
                          experts_per_token));
  HIP_CHECK(hipMalloc(&g_ext[device_index].n_local,
                      sizeof(int) * (size_t)expert_parallelism * batch_size));
  HIP_CHECK(hipMalloc(&g_ext[device_index].e_counts,
                      sizeof(int) * (size_t)n_experts));
  HIP_CHECK(hipMalloc(&g_ext[device_index].e_offsets,
                      sizeof(int) * ((size_t)n_experts + 1)));
  HIP_CHECK(hipMalloc(&g_ext[device_index].e_dev,
                      sizeof(int) * (size_t)expert_parallelism * batch_size *
                          experts_per_token));
  HIP_CHECK(hipMalloc(&g_ext[device_index].w_dev,
                      sizeof(float) * (size_t)expert_parallelism * batch_size *
                          experts_per_token));
  HIP_CHECK(hipMalloc(&g_ext[device_index].pair_pos,
                      sizeof(int) * (size_t)expert_parallelism * batch_size *
                          experts_per_token));
  HIP_CHECK(hipMalloc(&g_ext[device_index].z_partial,
                      sizeof(float) * (size_t)expert_parallelism * batch_size *
                          experts_per_token * hidden_dim));

  if (expert_parallelism > 1) {
    HIP_CHECK(hipMalloc(&g_ext[device_index].ext_t_bf16,
                        sizeof(__hip_bfloat16) * (size_t)expert_parallelism *
                            batch_size * hidden_dim));
    HIP_CHECK(hipMalloc(&g_ext[device_index].ext_topk_i,
                        sizeof(int) * (size_t)expert_parallelism * batch_size *
                            experts_per_token));
    HIP_CHECK(hipMalloc(&g_ext[device_index].ext_topk_v,
                        sizeof(float) * (size_t)expert_parallelism *
                            batch_size * experts_per_token));
    HIP_CHECK(hipMalloc(&g_ext[device_index].ext_router_score,
                        sizeof(float) * (size_t)expert_parallelism *
                            batch_size * n_experts));
    HIP_CHECK(hipMalloc(&g_ext[device_index].ext_t,
                        sizeof(float) * (size_t)expert_parallelism *
                            batch_size * hidden_dim));
  ...
  HIP_CHECK(hipMalloc(&g_ext[device_index].blk_counts,
                      sizeof(int) * (size_t)n_experts));
  HIP_CHECK(hipMalloc(&g_ext[device_index].blk_offsets,
                      sizeof(int) * (size_t)(n_experts + 1)));

  HIP_CHECK(hipMalloc(&g_ext[device_index].a_in,
                      sizeof(__hip_bfloat16) * (size_t)expert_parallelism *
                          batch_size * experts_per_token * hidden_dim));
  HIP_CHECK(hipMalloc(&g_ext[device_index].gate_up_bf16,
                      sizeof(__hip_bfloat16) * (size_t)expert_parallelism *
                          batch_size * experts_per_token * intermediate_dim));
  HIP_CHECK(
...
void ext_free_all(int n_devices) {
  for (int i = 0; i < n_devices; ++i) {
    ...
    if (g_ext[i].ext_router_score)
      HIP_CHECK(hipFree(g_ext[i].ext_router_score));
