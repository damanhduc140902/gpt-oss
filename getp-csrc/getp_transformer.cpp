...
static void upload_weights(TransformerWeights *w, DeviceTransformerWeights *dev_w, Config *cfg, 
  ...
  HIP_CHECK(hipSetDevice(device_index));
  {
    ...
    float *ptr = *_dev_data;
    dev_w->token_embedding_table = ptr; ptr += 1ll * cfg->vocab_size * cfg->hidden_dim;
    dev_w->out = ptr; ptr += 1ll * cfg->vocab_size * cfg->hidden_dim;
    dev_w->rms_attn_w = ptr; ptr += 1ll * n_layers * cfg->hidden_dim;
    dev_w->rms_ffn_w = ptr; ptr += 1ll * n_layers * cfg->hidden_dim;
    dev_w->rms_out_w = ptr; ptr += 1ll * cfg->hidden_dim;
    dev_w->w_qkv = ptr; ptr += 1ll * n_layers * cfg->hidden_dim * (head_dim * cfg->n_attn_heads + 2 * head_dim * cfg->n_kv_heads);
    dev_w->b_qkv = ptr; ptr += 1ll * n_layers * (head_dim * cfg->n_attn_heads + 2 * head_dim * cfg->n_kv_heads);
    dev_w->w_o = ptr; ptr += 1ll * n_layers * (head_dim * cfg->n_attn_heads) * cfg->hidden_dim;
    dev_w->b_o = ptr; ptr += 1ll * n_layers * cfg->hidden_dim;
    dev_w->attn_sinks = ptr; ptr += 1ll * n_layers * cfg->n_attn_heads;
    dev_w->w_router = ptr; ptr += 1ll * n_layers * cfg->hidden_dim * n_experts;
    dev_w->b_router = ptr; ptr += 1ll * n_layers * n_experts;
  }
  ...
  for (int l = 0; l < n_layers; ++l) {
    getp_memcpy_fp32_to_bf16(ptr, w->w_mlp1 + 1ll * l * n_experts * 2 * intermediate_dim * hidden_dim + 1ll * device_index * (n_experts / n_devices) * 2 * intermediate_dim * hidden_dim, (n_experts / n_devices) * 2 * intermediate_dim * hidden_dim);
    ptr += (n_experts / n_devices) * 2 * intermediate_dim * hidden_dim;
  }
  dev_w->b_mlp1 = ptr;
  for (int l = 0; l < n_layers; ++l) {
    getp_memcpy_fp32_to_bf16(ptr, w->b_mlp1 + 1ll * l * n_experts * 2 * intermediate_dim + 1ll * device_index * (n_experts / n_devices) * 2 * intermediate_dim, (n_experts / n_devices) * 2 * intermediate_dim);
    ptr += (n_experts / n_devices) * 2 * intermediate_dim;
  }
  dev_w->w_mlp2 = ptr;
  for (int l = 0; l < n_layers; ++l) {
    getp_memcpy_fp32_to_bf16(ptr, w->w_mlp2 + 1ll * l * n_experts * hidden_dim * intermediate_dim + 1ll * device_index * (n_experts / n_devices) * hidden_dim * intermediate_dim, (n_experts / n_devices) * hidden_dim * intermediate_dim);
    ptr += (n_experts / n_devices) * hidden_dim * intermediate_dim;
  }
  dev_w->b_mlp2 = ptr;
  for (int l = 0; l < n_layers; ++l) {
    getp_memcpy_fp32_to_bf16(ptr, w->b_mlp2 + 1ll * l * n_experts * hidden_dim + 1ll * device_index * (n_experts / n_devices) * hidden_dim, (n_experts / n_devices) * hidden_dim);
...
void init_device_run_state(RunState *s, Config *p) {
  ...
  HIP_CHECK(hipMalloc(&s->att, p->n_attn_heads * (p->seq_len + 1) * sizeof(float)));
  ...
  if (p->sliding_window > 0) {
    ...
    dim3 blockDim(32, 32);
    dim3 gridDim((p->seq_len + blockDim.x - 1) / blockDim.x, (p->seq_len + blockDim.y - 1) / blockDim.y);
