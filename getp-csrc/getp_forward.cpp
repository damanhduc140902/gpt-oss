...
__global__ void multihead_attention_kernel(
    ...
    int n_kv_heads, int pos, int seq_len, int apply_mask) {
  int h = blockDim.y * blockIdx.y + threadIdx.y;
...
void getp_multihead_attention(float *key_cache, float *value_cache,
                              ...
                              int seq_len, int sliding_window) {
  PROFILE_FUNCTION();
  ...
  multihead_attention_kernel<<<gridDim, blockDim>>>(
      ...
      n_attn_heads, n_kv_heads, pos, seq_len, apply_mask);
  HIP_CHECK(hipDeviceSynchronize());
}
...
void getp_weighted_sum(float *tb, float *value_cache, float *attn, int seq_len,
                       int n_attn_heads, int n_kv_heads, int pos,
                       int head_dim) {
  PROFILE_FUNCTION();
  dim3 blockDim(16, 16);
...
__global__ void softmax_kernel(float *A, int M, int N, int lda) {
  ...
  A += blockIdx.y *
       lda;  // offset to the row assigned to the corresponding block
...
// softmax matrix with shape MxN by row, and matrix A has leading dimension
// equal lda
...
void getp_swiglu(float *gate_up, float *gate, float *up, int intermediate_dim,
                 float swiglu_limit) {
  PROFILE_FUNCTION();
  dim3 blockDim(256);
  dim3 gridDim((intermediate_dim + blockDim.x - 1) / blockDim.x);
  swiglu_kernel<<<gridDim, blockDim>>>(gate_up, gate, up, intermediate_dim,
                                       swiglu_limit);
  HIP_CHECK(hipDeviceSynchronize());
}

void getp_split_gate_up(float *mlp1_out, float *gate, float *up,
                        int intermediate_dim) {
  PROFILE_FUNCTION();
  dim3 blockDim(256);
  dim3 gridDim((intermediate_dim + blockDim.x - 1) / blockDim.x);
  split_gate_up_kernel<<<gridDim, blockDim>>>(mlp1_out, gate, up,
                                              intermediate_dim);
  HIP_CHECK(hipDeviceSynchronize());
}
...
float *getp_forward(Transformer *transformer,
  ...
  for (unsigned long long l = 0; l < p->n_layers; l++) {
    ...
    getp_weighted_sum(dev_s->tb, dev_s->value_cache + loff, dev_s->att,
                      p->seq_len, p->n_attn_heads, p->n_kv_heads, pos,
                      head_dim);
