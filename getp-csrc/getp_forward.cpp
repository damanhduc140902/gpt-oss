...
template <typename T>
__device__ inline float to_float_dev(T v) { return static_cast<float>(v); }
template <>
__device__ inline float to_float_dev<__hip_bfloat16>(__hip_bfloat16 v) { return __bfloat162float(v); }
...
__global__ void gemv_rowmajor_cachex_kernel(float *__restrict__ y,
  ...
  for (int tile = 0; tile < n; tile += TILE_N) {
    ...
    if (out_row < d) {
      ...
      float partial = 0.f;
#pragma unroll 4
      for (int k = lane; k < tile_len; k += warpSize) partial += to_float_dev<T>(wrow[k]) * sX[k];
...
__global__ void gemv_qkv_rowmajor_cachex_kernel(float *__restrict__ q_out,
  ...
  for (int tile = 0; tile < n; tile += TILE_N) {
    ...
    if (out_row < d_total) {
      ...
      float partial = 0.f;
#pragma unroll 4
      for (int k = lane; k < tile_len; k += warpSize) partial += wrow[k] * sX[k];
#pragma unroll
      for (int off = warpSize >> 1; off > 0; off >>= 1) partial += __shfl_down(partial, off);
      acc += partial;
    }
    __syncthreads();
  }
  if (out_row < d_total && lane == 0) {
    ...
    float *dst = nullptr;
    int idx = 0;
    if (out_row < q_len) { dst = q_out; idx = out_row; }
    else if (out_row < q_len + k_len) { dst = k_out; idx = out_row - q_len; }
    else { dst = v_out; idx = out_row - q_len - k_len; }
    dst[idx] = acc + bias;
  }
}
...
__global__ void compute_inv_freq_kernel(float base, int head_dim, float scaling_factor,
...
__global__ void compute_cos_sin_kernel(float *cos_out, float *sin_out, float *inv_freq,
                                       float concentration, int pos, int d_half) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i >= d_half) return;
...
void getp_compute_cos_sin(int pos, float base, int head_dim, float scaling_factor,
  ...
  HIP_CHECK(hipMalloc(&inv_freq, d_half * sizeof(float)));
  {
    ...
    compute_inv_freq_kernel<<<gridDim, blockDim>>>(base, head_dim, scaling_factor, initial_context_length, ntk_beta, ntk_alpha, inv_freq);
  }
  {
    dim3 blockDim(256);
    dim3 gridDim((head_dim / 2 + blockDim.x - 1) / blockDim.x);
    compute_cos_sin_kernel<<<gridDim, blockDim>>>(cos_out, sin_out, inv_freq, concentration, pos, head_dim / 2);
  }
...
__global__ void softmax_kernel(float *A, int M, int N, int lda) {
  ...
  __syncthreads();
  for (int stride = blockDim.x / 2; stride > 0; stride /= 2) {
    if (tid < stride) { float t = smem[tid + stride]; if (t > smem[tid]) smem[tid] = t; }
    __syncthreads();
  }
...
__global__ void topk_kernel(float *router_score, float *topk_values, int *topk_indices, int n_experts, int experts_per_token) {
  int tid = threadIdx.x;
  if (tid >= experts_per_token) return;
  if (tid < n_experts) { topk_values[tid] = router_score[tid]; topk_indices[tid] = tid; }
  else { topk_values[tid] = -INFINITY; topk_indices[tid] = -1; }
  __syncthreads();
  for (int i = experts_per_token; i < n_experts; i++) {
    __syncthreads();
    float current_score = router_score[i];
    if (tid == 0) {
      int min_idx = 0; float min_val = topk_values[0];
      for (int j = 1; j < experts_per_token; j++) if (topk_values[j] < min_val) { min_val = topk_values[j]; min_idx = j; }
      if (current_score > min_val) { topk_values[min_idx] = current_score; topk_indices[min_idx] = i; }
    }
  }
}


__global__ void weighted_aggregate_kernel(float *e_agg, float *expert_out, float expert_weight, int hidden_dim) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i >= hidden_dim) return;
  atomicAdd(&e_agg[i], expert_out[i] * expert_weight);
}
void getp_topk(float *topk_values, int *topk_indices, float *router_score, int n_experts, int experts_per_token) {
  PROFILE_FUNCTION();
  dim3 blockDim(experts_per_token);
  dim3 gridDim(1);
  topk_kernel<<<gridDim, blockDim>>>(router_score, topk_values, topk_indices, n_experts, experts_per_token);
  HIP_CHECK(hipDeviceSynchronize());
}
...
__global__ void swiglu_fused_kernel(float *gate_up, const float *mlp1_out, int intermediate_dim, float swiglu_limit) {
  int i = blockDim.x * blockIdx.x + threadIdx.x;
...
void getp_swiglu_fused(float *gate_up, float *mlp1_out, int intermediate_dim, float swiglu_limit) {
  PROFILE_FUNCTION();
  dim3 block(256);
  dim3 grid((intermediate_dim + block.x - 1) / block.x);
  swiglu_fused_kernel<<<grid, block>>>(gate_up, mlp1_out, intermediate_dim, swiglu_limit);
  HIP_CHECK(hipDeviceSynchronize());
}
...
float *getp_forward(Transformer *transformer, DeviceTransformer **dev_transformeres, int token, int pos) {
  ...
  getp_compute_cos_sin(pos, p->rope_theta, head_dim, p->rope_scaling_factor, p->initial_context_length, ntk_beta, ntk_alpha, dev_cos_vals, dev_sin_vals);
