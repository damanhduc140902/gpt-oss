...
- **GQA with group size 8.** 64 query heads share 8 KV heads, `kv_mul = n_attn_heads / n_kv_heads`. The KV cache is therefore 8 × 64 = 512 wide per layer, not 4096 — an 8× saving that makes a 1536-sequence batch affordable. It also motivates assigning a thread block to a *KV* head rather than a query head in the attention kernel.
...
The HIP kernel never does that split. It reads weight rows `2*col` and `2*col+1` directly and unpacks the bias pair from a single 32-bit load, so gate and up for one channel arrive in one cache line and the SwiGLU is applied in registers before anything is written back. Keeping the file in the interleaved layout the checkpoint already uses therefore saves a full pass over a 5760-wide intermediate.
...
Ordering comes from a `CATEGORIES` list that mirrors the loader one-for-one; every safetensors key is bucketed by suffix (first match wins) and each bucket is sorted by the block index scraped out of `block.<N>.`. Ordering *inside* `CATEGORIES` is load-bearing: `attn.norm.scale` and `mlp.norm.scale` are listed before the bare `norm.scale` so that per-layer norms win the `endswith` race and only the final norm falls into the bare bucket.
...
| Everything else at 16 bits | 3.61 GB | 4.34 GB |
...
Normal tokens store their **raw bytes** and a `score` equal to the merge rank. Special tokens store their UTF-8 *name* (`<|start|>` and friends) and the sentinel score `-1e30`. The single exception is the NUL byte, exported as the literal stand-in `<0x00>` because it cannot travel through a C string.
...
- **The stored score does no work beyond that sentinel.** `get_merge_rank` returns `(unsigned)id` as the rank, not the score. This is correct only because `tiktoken`'s `mergeable_ranks` maps each byte string to an id that *is* its rank. It is worth knowing before porting this loader to another encoding.
...
The references are also the wrong *shape* for the runtime, which is a stronger statement than not being reproducible. Both `tests/references/output_{20b,120b}_token_ids.txt` hold exactly **4096 lines**, one completion each, and so do the two files under `tests/submission/`. But `getp` asserts `requests_per_model == EXPERT_PARALLELISM * BATCH_SIZE` at [`src/getp/run.cpp`](../src/getp/run.cpp) line 269, and with `n_parallel_models = n_devices / EXPERT_PARALLELISM` (line 510) that pins the total request count to `n_devices × 1536` for the 20B model and `n_devices × 768` for the 120B. On eight devices those are **12288** and **6144**. Neither is 4096, and no input file shipped in the tree declares either number — they declare 32, 256, 448, 3584 and 4096. So the 4096 references cannot line up one-for-one with any run this binary will accept at its shipped constants.
...
The reduction carries a packed `(value, index)` pair in one 64-bit word and merges tiles with a CAS loop:
...
Two values within 1e-6 relative are treated as tied, and the lower index wins. That is `atomic_update_max_pair` (around line 3192), the **cross-block** merge, and the router's top-k selection reuses the identical predicate — once in its per-thread scan and once in its shared-memory tree reduction (`router_topk_softmax_batch_kernel`, around lines 2124 and 2138).

The **intra-block** reduction inside `matmul_logits_argmax_bf16_kernel_tuned` looks like the same rule and is not. It is two passes over the same accumulators:

```c
// pass 1, around line 3390: value only, no epsilon
float mv = atomicMaxFloat(&smax[ly], v);
// pass 2, around line 3409: index, against the finished maximum
float mv = smax[ly];
...
The cross-block loop measures against a moving reference instead: a candidate is compared with whatever pair currently occupies the slot, which is not yet the global maximum. And because a tie updates the index but never the value — `take` only replaces the stored value when `v > ov + thr` — the value left in the pair can end up as much as one epsilon below the true maximum. That is harmless here because `extract_argmax_pairs` reads back `unpack_idx` and throws the value away, but it means the two predicates are not interchangeable: the intra-block one is the tighter, since it admits only columns within epsilon of a number that is already the maximum.

Both are deliberately **more tolerant** than the CPU, not stricter: CPU `sample_argmax` uses a plain `>` and so also keeps the lowest index on an *exact* tie, but it does not treat near-equal logits as tied. Without the epsilon, the order in which non-deterministic tile reductions accumulate could flip the argmax between runs for two logits that differ in the last bit.
...
A standalone `argmax_rows_kernel` exists in [`src/hip/forward.hip`](../src/hip/forward.hip) (around line 3451) but is never called; it is the unfused fallback. It uses the running-best predicate, the same one as `atomic_update_max_pair` and the router, not the two-pass form.
